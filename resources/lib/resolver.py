...
def resolve(url):
    ...
    resolver_obj = _get_resolver(url)
    value = None
    if resolver_obj is None:
        return None
    util.info('Using resolver \'%s\'' % str(resolver_obj.__name__))
    try:
        value = resolver_obj.resolve(url)
    except:
        traceback.print_exc()
    if value is None:
        return False
    ...
    [fix_stream(i, url, resolver_obj, default) for i in value]
