...
class NoRedirection(urllib.error.HTTPError):
...
def solve(url, cj, user_agent=None, wait=True):
    ...
    while tries < MAX_TRIES:
        ...
        # info('[CF] init: [{0}]'.format((init_equation.rstrip())))
        ...
        for equation in equations.split(';'):
            ...
            if len(equation) > len('.'.join(varname)):
                # debug('[CF] varname {0} line {1}'.format('.'.join(varname), equation))
                ...
                result = float(str(eval(str(result) + operator + str(solve_equation(
                    expression)))))
                #info('[CF] intermediate: %s = %s' % (equation, result))

        #debug('[CF] POCET: {0} {1}'.format(result, len(domain)))
        ...
        # info('[CF] url: %s' % url)
        # debug('[CF] headers: {0}'.format(headers))
        request = urllib.request.Request(url)
        for key in headers:
            request.add_header(key, headers[key])
        ...
        try:
            ...
            urllib.request.install_opener(opener)
            response = urllib.request.urlopen(request)
            # info('[CF] code: {}'.format(response.getcode()))
